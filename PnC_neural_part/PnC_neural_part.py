...
def main(args):
    ...
    for fold_idx in fold_idxs:
        ...
        print("[info] Total number of parameters is: {}".format(params))
        # intialise agent
        kwargs_agent = {'attr_mapping': attr_mapping,
        ...
        if args['mode'] == 'train':
            ...
            trainable_parameters_theta = [p for p in policy_network.parameters() if p.requires_grad]
            kargs_optim_theta = {'lr': args['lr_policy'],
                               'regularization': args['regularization'],
                               'scheduler': args['scheduler'],
                               'scheduler_mode': args['scheduler_mode'],
                               'decay_rate': args['decay_rate'],
                               'decay_steps': args['decay_steps'],
                               'patience': args['patience']}
            optim_phi, scheduler_phi = setup_optimization(trainable_parameters_phi,
                                                          optimiser_name='Adam',
                                                          **kargs_optim_phi)
            optim_theta, scheduler_theta = setup_optimization(trainable_parameters_theta,
                                                              optimiser_name='Adam',
                                                              **kargs_optim_theta)
        else:
            optim_phi, scheduler_phi, optim_theta, scheduler_theta = None, None, None, None
        if args['resume']:
            ...
            start_epoch = resume_training_phi_theta(resume_filename,
                                                    policy_network,
        ...
        train_compression_p_epoch, test_compression_p_epoch, val_compression_p_epoch, total_compression_p_epoch, total_compression_w_params_p_epoch, num_params_p_epoch = metrics
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_folder', type=str, default='PnC_neural_part_experiments')
    ...
    parser.add_argument('--input_node_embedding', type=str, default='None')
    parser.add_argument('--d_out_node_embedding', type=int, default=None)
    parser.add_argument('--edge_embedding', type=str, default='None')
    parser.add_argument('--d_out_edge_embedding', type=int, default=None)
    parser.add_argument('--inject_edge_features', type=parse.str2bool, default=False)
    parser.add_argument('--multi_embedding_aggr', type=str, default='sum')

    parser.add_argument('--degree_encoding', type=str, default='one_hot_max')
    ...
    parser.add_argument('--degree_as_tag', type=parse.str2bool, default=True)
    ...
    parser.add_argument('--bn_mlp', type=parse.str2bool, default=False)
    parser.add_argument('--bn', type=parse.str2bool, default=False)
    parser.add_argument('--activation', type=str, default='relu')
    parser.add_argument('--dropout', type=float, default=0)

    parser.add_argument('--num_layers', type=int, default=2)
    parser.add_argument('--model_name', type=str, default='MPNN_sparse')
    parser.add_argument('--aggr_fn', type=str, default='general')
    parser.add_argument('--d_out', type=int, default=16)
    parser.add_argument('--final_projection', type=parse.str2list2bool, default=[False])
    parser.add_argument('--final_projection_layer', type=str, default='mlp')
    parser.add_argument('--readout', type=str, default='sum')
#     parser.add_argument('--residual', type=parse.str2bool, default=False)
    #----------------- Neural Partitioning specifications
    parser.add_argument('--partitioning_algorithm', type=str, default='subgraph_selection')
    parser.add_argument('--sampling_mechanism', type=str, default='sampling_without_replacement')
    # deprecated
    # parser.add_argument('--sample_k', type=parse.str2bool, default=True)
    # parser.add_argument('--connected_sampling', type=parse.str2bool, default=True)
    ...
    parser.add_argument('--n_iters_train', type=int, default=None)
    parser.add_argument('--n_iters_test', type=int, default=None)
    ...
    parser.add_argument('--lr_dict', type=float, default=0.1)
